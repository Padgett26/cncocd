...
if ($login == 1) {
	...
	if (isset ( $_FILES ['newsletter'] ['name'] )) {
        ...
        <a href='http://cheyennecountycd.com/index.php?remove=$email&sec=$code' style='text-decoration:underline;'>Remove me from the mailing list.</a>
			...
			// Send
			mail ( $email, 'Cheyenne County Conservation District Newsletter', $message, $headers );
		}
	}
	...
	if ($_FILES ["image1"] ["size"] >= 1000) {
		...
		if ($width1 != null && $height1 != null) {
			...
			processPic ( $image1Name, $image1, $image1Type );
			$p1stmt = $db->prepare ( "UPDATE merch SET merchPic1=?, merchPic1Ext=? WHERE id=?" );
	...
	if ($_FILES ["image1"] ["size"] >= 1000) {
		...
		if ($width2 != null && $height2 != null) {
			...
			processPic ( $image2Name, $image2, $image2Type );
			$p2stmt = $db->prepare ( "UPDATE merch SET merchPic2=?, merchPic2Ext=? WHERE id=?" );
...
if ($login == 0) {
	...
	if (filter_input ( INPUT_GET, 'choice', FILTER_SANITIZE_STRING )) {
		...
		if ($choice == "news") {
			...
			$stmt = $db->prepare ( "SELECT id,title FROM news ORDER BY created DESC" );
			$stmt->execute ();
			while ( $row = $stmt->fetch () ) {
				$id = $row ['id'];
				$title = $row ['title'];
		...
		if ($choice == "addy") {
			$stmt = $db->prepare ( "SELECT id,phone,address,email FROM sitesettings" );
			$stmt->execute ();
			$row = $stmt->fetch ();
			$id = $row ['id'];
			$phone = $row ['phone'];
		...
		if ($choice == "tsssetting") {
			$stmt = $db->prepare ( "SELECT id,orderdate,pudate,tssyear,startdate,taxrate FROM sitesettings" );
			$stmt->execute ();
			$row = $stmt->fetch ();
			$id = $row ['id'];
