<?php
$stmt = $db->prepare ( "SELECT id,title,edutext,created FROM education ORDER BY title" );
$stmt->execute ();
$j = 1;
while ( $row = $stmt->fetch () ) {
	$id = $row ['id'];
	$title = $row ['title'];
	$edutext = nl2br ( make_links_clickable ( html_entity_decode ( $row ['edutext'], ENT_QUOTES ), $db ) );
	$created = $row ['created'];
	$j = $j + 1;
	echo <<<_END
