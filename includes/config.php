...
$p = (filter_input(INPUT_GET, 'page', FILTER_SANITIZE_STRING)) ? filter_input(
        INPUT_GET, 'page', FILTER_SANITIZE_STRING) : 'home';
$page = (file_exists($p . ".php")) ? $p : "home";

$stmt = $db->prepare(
        "SELECT address,phone,email,color1,color2,highLightColor FROM sitesettings");
$stmt->execute();
$row = $stmt->fetch();
$addy = $row['address'];
$phone = $row['phone'];
