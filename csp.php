<?php
$stmt = $db->prepare ( "SELECT id,title,deadline,csptext,created FROM csp ORDER BY title" );
$stmt->execute ();
$j = 1;
while ( $row = $stmt->fetch () ) {
	...
	$csptext = nl2br ( make_links_clickable ( html_entity_decode ( $row ['csptext'], ENT_QUOTES ), $db ) );
	$created = $row ['created'];
	$j = $j + 1;
	echo <<<_END
	    <br><br>
