<?php
$start = (filter_input(INPUT_GET, 'start', FILTER_SANITIZE_NUMBER_INT)) ? filter_input(
        INPUT_GET, 'start', FILTER_SANITIZE_NUMBER_INT) : 0;
$prev = $start - 15;
$next = $start + 15;
...
if ($login == "1") {
    if (filter_input(INPUT_POST, 'newphoto', FILTER_SANITIZE_NUMBER_INT) == 1) {
        ...
        $stmt1 = $db->prepare(
                "INSERT INTO photos VALUES" . "(NULL,?,?,?,'0','0')");
        $stmt1->execute(array(
                $caption,
                $created,
                $imageType
        ...
        $stmt2 = $db->prepare(
                "SELECT id FROM photos WHERE caption=? AND created=?");
        $stmt2->execute(array(
                $caption,
                $created
        ));
...
if ($login == "1") {
    ...
    echo "</td></tr>";
    $stmt = $db->prepare(
            "SELECT * FROM photos ORDER BY created DESC LIMIT $start,15");
    $stmt->execute();
